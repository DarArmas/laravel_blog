...
class CrearTablaMenusRoles extends Migration
{
    ...
    public function up()
    {
        Schema::create('menus_roles', function (Blueprint $table) {
            $table->unsignedBigInteger('menus_id'); //aqui no usamos la clase id de blueprint
            $table->foreign('menus_id', 'fk_menurol_menu')->references('id')->on('menus')->onDelete('cascade')->onUpdate('restrict'); //lave foranea
